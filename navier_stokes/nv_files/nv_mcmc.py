...
class NVMCMCDA(MCMCDA):
    ...
    def __init__(self, surrogate,observation_locations, observations_values, nparameters=2, 
                 ...
                 step_size=0.1,device="cpu",fs_indices_sol = None,fs_n = 128, fs_T=2,fs_steps =5e-4):
        
        super(NVMCMCDA, self).__init__(observation_locations, observations_values, nparameters, marginal,
        ...
        psm_solver =  torch_NVSolver2D(N=fs_n, L=2*np.pi, T=fs_T, nu=1e-2, 
                                       dt=fs_steps,num_sol=2, method='CN', 
                                       force= self.force_function,device=self.device)
        ...
        if marginal:
            methods = {dgala: self.dgala_marginal_model}
        else:
            methods = {torch_NVSolver2D: self.psm_model,
                        Vorticity: self.dgala_mean_model}
        ...
        self.fine_model = psm_solver
        self.finner_model = self.psm_model

    def force_function(self,X,Y):
        return  (torch.sin(X + Y) + torch.cos(X + Y))

    def psm_model(self, theta ):
        ...
        surg = self.fine_model.run_simulation(torch.tensor(w0[:,:,0]).to(self.device))
    ...
    def dgala_mean_model(self,theta):
        ...
        mean = self.surrogate_model.w(data.float()).detach()
    ...
    def dgala_marginal_model(self,theta):
        ...
        return mean[:,0].reshape(-1, 1).detach().double(), cov[:, :, 0].detach().double()
