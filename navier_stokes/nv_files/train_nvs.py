...
def ic_vort_samples(config):
    ...
    # Generate uniformly distributed values for `theta` in the range [-1, 1]
    theta = torch.rand(config.KL_expansion, 2, config.samples_size_initial) * 4 - 2  # Uniform(-1, 1)
...
def train_vorticity_dg(config,device):
    ...
    # Domain and sampler setup
    dom = torch.tensor([[0, 2 * torch.pi], [0, 2 * torch.pi],[0,config.time_domain]]).to(device)
    samples_interior = iter(UniformSampler(dom, batch_size_interior,device = device,rng_seed=config.seed))
...
def train_vorticity_epoch(config,device):
    ...
    for epoch in range(config.iterations):
        ...
        # Save the model checkpoint
        if (epoch % 100 == 0) and (epoch != 0):
            torch.save(dg_NVs, f"./navier_stokes/models/{wandb_config.name}.pth")

    # Save final model
    torch.save(dg_NVs, f"./navier_stokes/models/{wandb_config.name}.pth")
    wandb.save(f"./navier_stokes/models/{wandb_config.name}.pth")
