import torch
import torch.distributions as dist
import torch.multiprocessing as mp

#from pyro.distributions import Distribution

import numpy as np
from tqdm import tqdm  # For a progress bar
import random

#from Base.utilities import Timer

class MCMCDABase(torch.nn.Module):
    """
    Delayed Acceptance, first runs the MCMC then DAMCMC
    """
    def __init__(self,nparameters=2, iter_mcmc=1000000, iter_da = 20000,  burnin=None,               
                 proposal_type="random_walk",step_size=0.1, device="cpu"):
        super().__init__()

        # Likelihood data
        self.device = device
        self.nparameters = nparameters

        self.outer_likelihood_value = None
        self.inner_likelihood_value = None
        
        # MCMC settings
        self.iter_da = iter_da
        self.iter_mcmc = iter_mcmc
        self.proposal_type = proposal_type
        self.burnin = int(self.iter_mcmc * 0.25) if burnin is None else burnin
        self.dt = torch.tensor(step_size, dtype=torch.float64, device=self.device)  # Step size for proposals
        self.ideal_acceptace_rate = torch.tensor( 0.234, dtype=torch.float64, device=self.device)

        #MCMC Data to Store 
        #self.theta = torch.rand(self.nparameters, device=self.device, dtype=torch.float64) * 2 - 1

        self.to(device)

    def allocate_mcmc_buffers(self):
        self.mcmc_samples = torch.zeros((self.iter_mcmc, self.nparameters), device=self.device)
        self.lh_mcmc_evaluations =  torch.zeros((self.iter_mcmc, self.nobservations), device=self.device)
        # self.mcmc_samples = torch.zeros((self.iter_mcmc + self.burnin, self.nparameters), device=self.device)
        # self.lh_mcmc_evaluations =  torch.zeros((self.iter_mcmc + self.burnin, self.nobservations), device=self.device)


    def allocate_da_buffers(self):
        #self.da_samples =  torch.zeros((self.iter_da,self.nparameters), device=self.device)
        # self.lh_da_outer = torch.zeros((self.iter_da,self.nobservations), device=self.device)
        # self.lh_da_inner = torch.zeros((self.iter_da,self.nobservations), device=self.device)
        self.da_acc_rej = torch.zeros((self.iter_da), device=self.device)

    def log_prior(self, theta):
        """Directly call the precomputed likelihood function."""
        return NotImplementedError("log_likelihood must be implemented in a subclass.")

    def log_likelihood_outer(self, theta):
        """Define the likelihood function for the coarse model. Must be overridden."""
        raise NotImplementedError("log_likelihood must be implemented in a subclass.")
    
    def log_likelihood_inner(self, theta):
        """Define the likelihood function for the finner model. Must be overridden."""
        raise NotImplementedError("log_likelihood must be implemented in a subclass.")
    
    def posterior_distribution_outer(self, theta):
        """Directly call the precomputed likelihood function."""
        return NotImplementedError("log_likelihood must be implemented in a subclass.")

    def posterior_distribution_inner(self, theta):
        """Directly call the precomputed likelihood function."""
        return NotImplementedError("log_likelihood must be implemented in a subclass.")


    def proposal(self, theta, dt):
        """Proposal with independent step sizes for each chain."""
        if self.proposal_type == "random_walk":
            return theta + torch.normal(mean=torch.zeros_like(theta), std=dt)  # Scale noise by dt (per chain)
        elif self.proposal_type == "pCN":
            return theta*torch.sqrt(1-dt**2) + torch.normal(mean=torch.zeros_like(theta), std=dt)  

    def run_chain(self, verbose=True):
        """Run Metropolis-Hastings """
        self.allocate_mcmc_buffers()
        print("Starting MCMC....")

        #MCMC Data to Store 
        self.theta = torch.rand(self.nparameters, device=self.device, dtype=torch.float64) * 2 - 1

        accepted_proposals = 0
 
        log_posterior,_ = self.posterior_distribution_outer(self.theta) 
        # Initialize separate dt for each chai
   
        if verbose:
            pbar = tqdm(range(self.iter_mcmc + self.burnin), desc="Running MCMC", unit="step")
        else:
            pbar = range(self.iter_mcmc + self.burnin)

        for i in pbar:
            theta_proposal = self.proposal(self.theta, self.dt)  # Pass dt to proposal

            log_posterior_proposal,_ = self.posterior_distribution_outer(theta_proposal)

            # Compute acceptance probabilities (vectorized)
            diff = log_posterior_proposal - log_posterior
            a = diff.exp().clamp(max=1.0)
            
            if torch.rand((), device=self.device) < a:
            #if torch.rand(1, device=self.device) < a:
                self.theta = theta_proposal
                accepted_proposals += 1
                log_posterior = log_posterior_proposal

            # Adaptive step size adjustment
            self.dt += self.dt * (a - self.ideal_acceptace_rate) / (i + 1)

            # Store samples
            if i > self.burnin:
                self.mcmc_samples[i -self.burnin , :] = self.theta.detach()
                self.lh_mcmc_evaluations[i - self.burnin,:] = self.outer_likelihood_value.flatten()

            if self.proposal_type == "pCN":
                self.dt = self.dt.clamp(max=1.0)

            if verbose and (i % (self.iter_mcmc // 10) == 0)and (i!=0):
                pbar.set_postfix(acceptance_rate=f"{accepted_proposals / (i+1):.4f}", proposal_variance=f"{self.dt:.4f}")

  
    def run_da(self, verbose=True):
        """Run DA Montecarlo """
        self.allocate_da_buffers()
        print("Starting DA Montecarlo....")
        self.theta = torch.rand(self.nparameters, device=self.device, dtype=torch.float64) * 2 - 1

        if self.fine_model is None:
            raise RuntimeError("DA needs a fine model")

        if verbose:
            pbar = tqdm(range(self.iter_da), desc="Running Delayed Acceptance", unit="step")
    
        #theta = self.theta
        inner_accepted,inner_mh = 0,0
        log_posterior_outer,llh_outer = self.posterior_distribution_outer(self.theta) 
        llh_inner = self.log_likelihood_inner(self.theta) 
        fine_chain =  []
        proposals_outer = 0

        while inner_mh < self.iter_da:
            # Propose new theta values
            theta_proposal = self.proposal(self.theta,self.dt)
            proposals_outer +=1
            
            # Compute the current log-posterior
            log_posterior_proposal_outer, llh_proposal_outer = self.posterior_distribution_outer(theta_proposal)
            # Compute the acceptance ratio
            diff = log_posterior_proposal_outer - log_posterior_outer
            a = diff.exp().clamp(max=1.0)

            # Accept or reject the proposal
            if torch.rand(1, device=self.device) < a:
                inner_mh += 1

                llh_proposal_inner = self.log_likelihood_inner(theta_proposal)
                # Compute the acceptance ratio
                diff = (llh_proposal_inner - llh_inner) - (llh_proposal_outer - llh_outer)
                a2 = diff.exp().clamp(max=1.0)
                #print(llh_proposal_inner,llh_inner,llh_proposal_outer,llh_outer,diff,a)
                if torch.rand(1, device=self.device) < a2:
                    self.theta = theta_proposal
                    self.da_acc_rej[inner_mh-1] +=1
                    inner_accepted += 1

                    log_posterior_outer,llh_outer = log_posterior_proposal_outer, llh_proposal_outer
                    llh_inner = llh_proposal_inner

                # Adaptive step size adjustment (each chain updates its own dt)
                #self.dt += self.dt * (a2 - 0.25) / (inner_mh)

                 # Save results of the values of the surrogates and solver and proposed thetas
                # self.da_samples[inner_mh-1,:] = theta_proposal.clone()
                # self.lh_da_outer[inner_mh-1,:] = self.outer_likelihood_value.T.clone()
                # self.lh_da_inner[inner_mh-1,:]  = self.inner_likelihood_value.T.clone()

                if verbose:
                    pbar.update(1)

            fine_chain.append(self.theta) 

            # Update progress bar and print progress every 10% (or any interval)
            if verbose and (inner_mh % (self.iter_da // 10) == 0) and (inner_mh != 0):
                acceptance_rate = inner_accepted / inner_mh
                pbar.set_postfix(acceptance_rate=f"{acceptance_rate:.4f}", proposal_variance=f"{self.dt:.4f}")

        self.fine_model_chain = torch.tensor(np.array(fine_chain), dtype=torch.float64, device=self.device)
        # End progress bar
        if verbose:
            pbar.close()
            print(f"Times inner step {inner_mh:.4f}, Acceptance Rate: {inner_accepted / inner_mh:.4f},Acceptance Rate First Step {inner_mh/ proposals_outer:.4f}")

    # def run_da_(self, verbose=True):
    #     """Run DA Montecarlo """
    #     self.allocate_da_buffers()
    #     print("Starting DA Montecarlo....")
    #     if self.fine_model is None:
    #         raise RuntimeError("DA needs a fine model")

    #     if verbose:
    #         pbar = tqdm(range(self.iter_da), desc="Running Delayed Acceptance", unit="step")
        
    #     theta = self.theta
    #     inner_accepted,inner_mh = 0,0
    #     log_posterior_outer,llh_outer = self.posterior_distribution_outer(theta) 
    #     llh_inner = self.log_likelihood_inner(theta)
    #     llh_outer_inner = llh_outer

    #     while inner_mh < self.iter_da:
    #         # Propose new theta values
    #         theta_proposal_outer = self.proposal(theta,self.dt)
            
    #         # Compute the current log-posterior
    #         log_posterior_proposal_outer, llh_proposal_outer = self.posterior_distribution_outer(theta_proposal_outer)
    #         # Compute the acceptance ratio
    #         diff = log_posterior_proposal_outer - log_posterior_outer
    #         a = diff.exp().clamp(max=1.0)

    #         # Accept or reject the proposal
    #         if torch.rand(1, device=self.device) < a:
    #             inner_mh += 1

    #             llh_proposal_inner = self.log_likelihood_inner(theta_proposal_outer)
    #             # Compute the acceptance ratio
    #             diff = (llh_proposal_inner - llh_inner) - (llh_proposal_outer - llh_outer_inner)
    #             a = diff.exp().clamp(max=1.0)

    #             theta = theta_proposal_outer
    #             log_posterior_outer,llh_outer = log_posterior_proposal_outer, llh_proposal_outer

    #             if torch.rand(1, device=self.device) < a:
    #                 # self.theta_inner = theta_proposal_outer
    #                 self.da_acc_rej[inner_mh-1] +=1
    #                 inner_accepted += 1

    #                 llh_outer_inner = llh_proposal_outer
    #                 llh_inner = llh_proposal_inner

    #              # Save results of the values of the surrogates and solver and proposed thetas
    #             self.da_samples[inner_mh-1,:] = theta_proposal_outer.clone()
    #             self.lh_da_outer[inner_mh-1,:] = self.outer_likelihood_value.T.clone()
    #             self.lh_da_inner[inner_mh-1,:]  = self.inner_likelihood_value.T.clone()

    #             if verbose:
    #                 pbar.update(1)

    #         # Update progress bar and print progress every 10% (or any interval)
    #         if verbose and (inner_mh % (self.iter_da // 10) == 0) and (inner_mh != 0):
    #             acceptance_rate = inner_accepted / inner_mh
    #             pbar.set_postfix(acceptance_rate=f"{acceptance_rate:.4f}")
    #     # End progress bar
    #     if verbose:
    #         pbar.close()
    #         print(f"Times inner step {inner_mh:.4f}, Acceptance Rate: {inner_accepted / inner_mh:.4f}")

class MCMCDA(MCMCDABase):
    """
    Delayed Acceptance, first runs the MCMC then DAMCMC
    """
    def __init__(self, observation_locations, observations_values, nparameters=2, marginal=False,
                 observation_noise=0.5, iter_mcmc=1000000, iter_da = 20000,  burnin=None,               
                 proposal_type="random_walk",prior_type = "uniform" ,uniform_limit=1,my_reg = 1e-3,
                 step_size=0.1, device="cpu"):
        super().__init__(nparameters,iter_mcmc,iter_da,burnin,proposal_type,step_size, device)
       
        # Likelihood data
        self.observation_locations = torch.tensor(observation_locations, dtype=torch.float64, device=self.device)
        self.observations_values = torch.tensor(observations_values, dtype=torch.float64, device=self.device)
        self.nobservations = self.observations_values.shape[0]
        self.observation_noise =  torch.tensor(observation_noise, dtype=torch.float64, device=self.device)

        if prior_type == "moreau":
            self.my_reg = torch.tensor(my_reg, device=device)
            self.my_scaling_term = 1 / (2 + torch.sqrt(2 * torch.pi * self.my_reg)).to(self.device)  # Calculate once
        elif prior_type == "normal":
            pcn_mu = torch.zeros(self.nparameters,device=self.device)
            pcn_cov= torch.eye(self.nparameters,device=self.device)
            self.normal_dist = dist.MultivariateNormal(pcn_mu, pcn_cov)
        elif prior_type == "uniform":
            self.uniform_limit = torch.tensor(uniform_limit, dtype=torch.float64, device=self.device)
        else:
            raise ValueError(f"Prior for proposal type '{prior_type}' is not supported.")        
    
        # Dictionary to map surrogate classes to their likelihood functions
        prior_methods = {"uniform": self.log_uniform,
                        "normal": self.log_normal,
                        "moreau": self.log_MY_envelope}
        
        if proposal_type == "pCN":
            self.log_prior = lambda theta: 0.0
        elif prior_type in prior_methods:
            self.log_prior = prior_methods[prior_type]
        else:
            raise ValueError(f"Prior type '{prior_type}' is not supported.")  
        
        options = {"marginal": self.log_marginal_likelihood_outer,
                   "mean": self.log_mean_likelihood_outer}
        
        mode = "marginal" if marginal else "mean"
        self.log_likelihood_outer = options[mode]
        
        self.to(device)
   
    def posterior_distribution_outer(self,theta):
        ll = self.log_likelihood_outer(theta)
        lp = self.log_prior(theta)
        return lp + ll, ll  # posterior, likelihood

    def log_MY_envelope(self, theta):
        """
        Log prior with Moreau-Yosida regularization for the uniform distribution between -1 and 1.
        """
        # Regularization term for all theta values
        regularization_term = -(torch.clamp(torch.abs(theta) - 1, min=0) ** 2) / (2 * self.my_reg)
        return regularization_term + torch.log(self.my_scaling_term)

    def log_uniform(self, theta):
        if not torch.logical_and(theta >= -self.uniform_limit, theta <= self.uniform_limit).all():
            return torch.tensor(-float("inf"), device=self.device)  # Ensure it remains a tensor
        return torch.tensor(0.0, device=self.device)  # Keep consistency
        
    def log_normal(self, theta):
        return self.normal_dist.log_prob(theta)
    
    def log_mean_likelihood_outer(self,theta):
        mean_prediction = self.outer_model(theta)
        self.outer_likelihood_value = mean_prediction.detach()
        return -0.5 * torch.sum(((self.observations_values - mean_prediction) ** 2) / (self.observation_noise ** 2))

    def log_marginal_likelihood_outer(self,theta):
        mean_prediction, covariance = self.outer_model(theta)
        self.outer_likelihood_value = mean_prediction

        diff = (self.observations_values - mean_prediction)

        sigma = covariance +  torch.eye(self.nobservations, device=self.device) *  self.observation_noise ** 2

        L = torch.linalg.cholesky(sigma)
        k_inv_g = torch.cholesky_solve(diff, L)  

        #cte = 0.5 * (self.nobservations * torch.log(torch.tensor(2 * torch.pi)) + 2.0 * torch.sum(torch.log(torch.diag(L))))
        cte = torch.sum(torch.log(torch.diag(L)))
        # Log-likelihood
        lg = -0.5 *(diff.T @ k_inv_g) - cte
        return lg.squeeze()

    def log_likelihood_inner(self, theta):
        mean_prediction = self.finner_model(theta)
        self.inner_likelihood_value = mean_prediction
        return -0.5 * torch.sum(((self.observations_values - mean_prediction) ** 2) / (self.observation_noise ** 2))

class MCMCDAtoy(MCMCDABase):
    """
    Delayed Acceptance, first runs the MCMC then DAMCMC
    """
    def __init__(self, mu_coarse, sigma_coarse,mu_fine, sigma_fine,  iter_mcmc=1000000, iter_da = 20000,  burnin=None,               
                 proposal_type="random_walk",nparameters = 1,
                 step_size=0.1, device="cpu"):
        super().__init__(nparameters,iter_mcmc,iter_da,burnin,proposal_type,step_size, device)
       
        # Likelihood data
        self.mu_coarse = torch.tensor(mu_coarse, dtype=torch.float64, device=self.device)
        self.sigma_coarse =  torch.tensor(sigma_coarse, dtype=torch.float64, device=self.device)

        self.mu_fine = torch.tensor(mu_fine, dtype=torch.float64, device=self.device)
        self.sigma_fine =  torch.tensor(sigma_fine, dtype=torch.float64, device=self.device)
        self.nobservations = 0
        self.outer_likelihood_value = torch.tensor(0)
        self.inner_likelihood_value = torch.tensor(0)

        self.fine_model = 0
        self.to(device)

    def posterior_distribution_outer(self,theta):
        lp = -0.5 * torch.log(2 * torch.pi * self.sigma_coarse**2) \
             - 0.5 * ((theta - self.mu_coarse)**2) / (self.sigma_coarse**2)
        return lp.squeeze(), lp.squeeze()
    
    def log_likelihood_inner(self, theta):
        return -0.5 * torch.log(2 * torch.pi * self.sigma_fine**2) \
               - 0.5 * ((theta - self.mu_fine)**2) / (self.sigma_fine**2)

# class MetropolisHastings(torch.nn.Module):
#     """Implements Metropolis Hastings with multiple chains and configurable prior, likelihood, and proposal."""
    
#     def __init__(self, observation_locations, observations_values, nparameters=2, 
#                  observation_noise=0.5, nsamples=1000000, burnin=None,  
#                  proposal_type="random_walk", step_size=0.1, 
#                  uniform_limit=1,my_reg = 1e-3,device="cpu"):
#         super(MetropolisHastings, self).__init__()

#         # Likelihood data to torch data
#         self.device = device
#         self.observation_locations = torch.tensor(observation_locations, dtype=torch.float64, device=self.device)
#         self.observations_values = torch.tensor(observations_values, dtype=torch.float64, device=self.device)
#         self.observation_noise = observation_noise
#         self.nparameters = nparameters
        
#         # MCMC settings
#         self.nsamples = nsamples
#         self.burnin = int(self.nsamples * 0.25) if burnin is None else burnin
#         self.proposal_type = proposal_type
#         self.uniform_limit = torch.tensor(uniform_limit, dtype=torch.float64, device=self.device)
#         #self.ideal_acceptace_rate = 0.234 if self.proposal_type == "random_walk" else 0.576
#         self.ideal_acceptace_rate = torch.tensor( 0.234, dtype=torch.float64, device=self.device)
#         self.dt = torch.tensor(step_size, dtype=torch.float64, device=self.device)  # Step size for proposals

#         # Langevin not implemented
#         if proposal_type == "langevin":
#             self.my_reg = torch.tensor(my_reg, device=device)
#             self.my_scaling_term = 1 / (2 + torch.sqrt(2 * torch.pi * self.my_reg)).to(self.device)  # Calculate once
        
#         # Normal prior 
#         self.pcn_mu = torch.zeros(self.nparameters,device=self.device)
#         self.pcn_cov= torch.eye(self.nparameters,device=self.device)
#         self.normal_dist = dist.MultivariateNormal(self.pcn_mu, self.pcn_cov)

#         # Dictionary to map proposal types to their prior functions
#         prior_methods = {
#             "random_walk": self.log_uniform,
#             "langevin": self.log_MY_envelope,
#             # "pCN" intentionally omitted
#         }

#         if proposal_type == "pCN":
#             self.log_prior_func = lambda theta: (0.0, None)
#         elif proposal_type in prior_methods:
#             self.log_prior_func = prior_methods[proposal_type]
#         else:
#             raise ValueError(f"Prior for proposal type '{proposal_type}' is not supported.")

#         self.to(device)

#     def log_MY_envelope(self, theta):
#         """
#         Log prior with Moreau-Yosida regularization for the uniform distribution between -1 and 1.
#         """
#         # Regularization term for all theta values
#         regularization_term = -(torch.clamp(torch.abs(theta) - 1, min=0) ** 2) / (2 * self.my_reg)
#         return regularization_term + torch.log(self.my_scaling_term)
    
#     # Log prior
#     def log_normal(self, theta):
#         return self.normal_dist.log_prob(theta), torch.tensor(0.0) 

#     def log_uniform(self, theta):
#         if not torch.logical_and(theta >= -self.uniform_limit, theta <= self.uniform_limit).all():
#             return torch.tensor(-float("inf")),torch.tensor(0.0)  # Ensure it remains a tensor
#         return torch.tensor(0.0),torch.tensor(0.0)  # Keep consistency

#     # def log_prior(self, theta):
#     #     """Directly call the precomputed likelihood function."""
#     #     return self.log_prior_func(theta)

#     def log_likelihood(self, theta):
#         """Define the likelihood function. Must be overridden."""
#         raise NotImplementedError("log_likelihood must be implemented in a subclass.")
    
#     def MYULA(self, theta):
#         """Define the likelihood function. Must be overridden."""
#         raise NotImplementedError("log_likelihood must be implemented in a subclass.")

#     def proposal(self, theta, dt):
#         """Proposal with independent step sizes for each chain."""
#         if self.proposal_type == "random_walk":
#             return theta + torch.normal(mean=torch.zeros_like(theta), std=dt)  # Scale noise by dt (per chain)
        
#         elif self.proposal_type == "pCN":
#             return theta*torch.sqrt(1-dt**2) + torch.normal(mean=torch.zeros_like(theta), std=dt)  
        
#         elif self.proposal_type == "langevin":
#             return theta + dt * self.MYULA(theta) + torch.sqrt(2*dt) * torch.randn_like(theta)
        
#     def posterior_distribution(self, theta):
#         loglike, surg = self.log_likelihood(theta)
#         logprior, _ = self.log_prior_func(theta)  # safe: always returns a tuple
#         posterior = logprior + loglike
#         return posterior, surg
    
#     def run_chain(self, verbose=True, eval_val = False):
#         """Run Metropolis-Hastings """
#         #theta = torch.empty((self.nparameters), device=self.device).uniform_(-1, 1)
#         theta = torch.rand(self.nparameters, device=self.device, dtype=torch.float64) * 2 - 1
#         samples = torch.zeros((self.nsamples + self.burnin, self.nparameters), device=self.device)
#         evaluations = torch.zeros((self.nsamples + self.burnin, self.observations_values.shape[0]), device=self.device)

#         accepted_proposals = 0

#         log_posterior, model_eval_post = self.posterior_distribution(theta) 
#         # Initialize separate dt for each chai
#         dt =  self.dt
        
#         if verbose:
#             pbar = tqdm(range(self.nsamples + self.burnin), desc="Running MCMC", unit="step")
#         else:
#             pbar = range(self.nsamples + self.burnin)

#         for i in pbar:
#             theta_proposal = self.proposal(theta, dt)  # Pass dt to proposal

#             log_posterior_proposal,model_eval_proposal = self.posterior_distribution(theta_proposal)

#             # Compute acceptance probabilities (vectorized)
#             a = torch.exp(log_posterior_proposal - log_posterior).clamp(max=1.0)

#             if torch.rand(1, device=self.device) < a:
#                 theta = theta_proposal
#                 accepted_proposals += 1
#                 log_posterior = log_posterior_proposal
#                 model_eval_post = model_eval_proposal

#             # Store samples
#             samples[i, :] = theta

#             if eval_val:
#                 evaluations[i,:] = model_eval_post.flatten()

#             # Adaptive step size adjustment (each chain updates its own dt)
#             dt += dt * (a - self.ideal_acceptace_rate) / (i + 1)

#             if self.proposal_type == "pCN":
#                 dt = dt.clamp(max=1.0)

#             if verbose and (i % (self.nsamples // 10) == 0)and (i!=0):
#                 pbar.set_postfix(acceptance_rate=f"{accepted_proposals / (i+1):.4f}", proposal_variance=f"{dt:.4f}")

#         return samples[self.burnin:,:].detach().cpu().numpy(),accepted_proposals/self.nsamples, dt,samples[-1,:],evaluations[self.burnin:,:].detach().cpu().numpy()

    # Run chain in parallel
    # def _run_chain(self, seed, result_queue):
    #     """Runs a single chain with a given random seed (for parallel execution)."""
    #     torch.manual_seed(seed)
    #     samples, accepted_proposals = self.run_chain(verbose=False)
    #     result_queue.put((samples, accepted_proposals))

    # def run_chains(self,nchains = 2):
    #     """Run multiple chains in parallel or sequentially."""
    #     seeds = [torch.randint(0, 100000, (1,)).item() for _ in range(nchains)]
    #     processes = []
    #     result_queue = mp.Queue()

    #     for i in range(nchains):
    #         p = mp.Process(target=self._run_chain, args=(seeds[i], result_queue))
    #         processes.append(p)
    #         p.start()

    #     for p in processes:
    #         p.join()

    #     results = []
    #     while not result_queue.empty():
    #         results.append(result_queue.get())

    #     return results



# class MCMCDA(torch.nn.Module):
#     """
#     Delayed Acceptance, first runs the MCMC then DAMCMC
#     """
#     def __init__(self, observation_locations, observations_values,mcmc_class, nparameters=2, 
#                  observation_noise=0.5, iter_mcmc=1000000, iter_da = 20000,                 
#                  proposal_type="random_walk",uniform_limit=1,my_reg = 1e-3,
#                  step_size=0.1, device="cpu"):
#         super(MCMCDA, self).__init__()

#         # Likelihood data
#         self.device = device
#         self.observation_locations = torch.tensor(observation_locations, dtype=torch.float64, device=self.device)
#         self.observations_values = torch.tensor(observations_values, dtype=torch.float64, device=self.device)
#         self.observation_noise = observation_noise
#         self.nparameters = nparameters

#         self.outer_likelihood_value = None
#         self.inner_likelihood_value = None
        
#         # MCMC settings
#         self.iter_da = iter_da
#         self.iter_mcmc = iter_mcmc
#         self.proposal_type = proposal_type
#         self.uniform_limit = torch.tensor(uniform_limit, dtype=torch.float64, device=self.device)

#         self.ideal_acceptace_rate = torch.tensor( 0.234, dtype=torch.float64, device=self.device)
#         self.dt = torch.tensor(step_size, dtype=torch.float64, device=self.device)  # Step size for proposals

#         if proposal_type == "langevin":
#             self.my_reg = torch.tensor(my_reg, device=device)
#             self.my_scaling_term = 1 / (2 + torch.sqrt(2 * torch.pi * self.my_reg)).to(self.device)  # Calculate once
        
#         self.pcn_mu = torch.zeros(self.nparameters,device=self.device)
#         self.pcn_cov= torch.eye(self.nparameters,device=self.device)
#         self.normal_dist = dist.MultivariateNormal(self.pcn_mu, self.pcn_cov)

#         # Dictionary to map surrogate classes to their likelihood functions
#         prior_methods = {"random_walk": self.log_uniform,
#                          "langevin": self.log_MY_envelope
#                         }
        
#         if proposal_type == "pCN":
#             self.log_prior_func = None  # Or set to a dummy function that returns 0
#             self.posterior_distribution_outer = self.log_likelihood_outer
#             self.posterior_distribution_inner = self.log_likelihood_inner

#         elif proposal_type in prior_methods:
#             self.log_prior_func = prior_methods[proposal_type]
#             self.posterior_distribution_outer= (lambda theta: self.log_prior_func(theta) + self.log_likelihood_outer(theta))
#             self.posterior_distribution_inner= (lambda theta: self.log_prior_func(theta) + self.log_likelihood_inner(theta))
#         else:
#             raise ValueError(f"Prior for proposal type '{proposal_type}' is not supported.")        
        
#         self.mcmc_chain = mcmc_class

#         self.to(device)


#     def log_MY_envelope(self, theta):
#         """
#         Log prior with Moreau-Yosida regularization for the uniform distribution between -1 and 1.
#         """
#         # Regularization term for all theta values
#         regularization_term = -(torch.clamp(torch.abs(theta) - 1, min=0) ** 2) / (2 * self.my_reg)
#         return regularization_term + torch.log(self.my_scaling_term)

#     def log_uniform(self, theta):
#         if not torch.logical_and(theta >= -self.uniform_limit, theta <= self.uniform_limit).all():
#             return torch.tensor(-float("inf"))  # Ensure it remains a tensor
#         return torch.tensor(0.0)  # Keep consistency
        
#     def log_normal(self, theta):
#         return self.normal_dist.log_prob(theta)

#     def log_prior(self, theta):
#         """Directly call the precomputed likelihood function."""
#         return self.log_prior_func(theta)

#     def log_likelihood_outer(self, theta):
#         """Define the likelihood function for the coarse model. Must be overridden."""
#         raise NotImplementedError("log_likelihood must be implemented in a subclass.")
    
#     def log_likelihood_inner(self, theta):
#         """Define the likelihood function for the finner model. Must be overridden."""
#         raise NotImplementedError("log_likelihood must be implemented in a subclass.")

#     def proposal(self, theta, dt):
#         """Proposal with independent step sizes for each chain."""
#         if self.proposal_type == "random_walk":
#             return theta + torch.normal(mean=torch.zeros_like(theta), std=dt)  # Scale noise by dt (per chain)
#         elif self.proposal_type == "pCN":
#             return theta*torch.sqrt(1-dt**2) + torch.normal(mean=torch.zeros_like(theta), std=dt)  
        
#         # elif self.proposal_type == "langevin":
#         #     if not theta.requires_grad:
#         #         theta.requires_grad_()  # Ensure theta is differentiable
#         #     gradient = torch.autograd.grad(self.log_likelihood(theta), theta, retain_graph=True)[0]
#         #     return theta + 0.5 * dt * gradient + dt * torch.randn_like(theta)

#     # def posterior_distribution_outer(self,theta):

#     #     if self.proposal_type == "pCN":
#     #         return self.log_likelihood_outer(theta)
#     #     else:
#     #         return self.log_prior(theta) + self.log_likelihood_outer(theta)
        
#     # def posterior_distribution_inner(self,theta):
#     #     if self.proposal_type == "pCN":
#     #         return self.log_likelihood_inner(theta)
#     #     else:
#     #         return self.log_prior(theta) + self.log_likelihood_inner(theta)


#     def run_chain(self, verbose=True):
#         """Run Metropolis-Hastings """
#         acceptance_list = torch.zeros((self.iter_da), device=self.device)
        
#         theta_inner =  torch.zeros((self.iter_da,self.nparameters), device=self.device)
#         likelihoods_val_nn = torch.zeros((self.iter_da,self.observation_locations.shape[0]), device=self.device)
#         likelihoods_val_solver = torch.zeros((self.iter_da,self.observation_locations.shape[0]), device=self.device)

#         print("Starting MCMC....")

#         # outer_timer = Timer(use_gpu=True)
#         # outer_timer.start()
#         samples_outer, _, dt,theta,_ = self.mcmc_chain.run_chain(verbose = verbose)
#         theta = theta.double()  # convert to float64
#         print("Starting Delayed Acceptance....")

#         if verbose:
#             pbar = tqdm(range(self.iter_da), desc="Running Delayed Acceptance", unit="step")
    
#         inner_accepted,inner_mh = 0,0
#         # inner_timer = Timer(use_gpu=True)
#         # inner_timer.start()
        
#         log_posterior_outer = self.posterior_distribution_outer(theta) 
#         log_posterior_inner = self.posterior_distribution_inner(theta) 

#         while inner_mh < self.iter_da:
#             # Propose new theta values
#             theta_proposal = self.proposal(theta,dt)
            
#             # Compute the current log-posterior
#             log_posterior_proposal_outer = self.posterior_distribution_outer(theta_proposal)
#             # Compute the acceptance ratio
#             a = torch.clamp(torch.exp(log_posterior_proposal_outer - log_posterior_outer), max=1.0)
#             a_rec = torch.clamp(torch.exp(log_posterior_outer - log_posterior_proposal_outer), max=1.0)
#             # Accept or reject the proposal
#             if torch.rand(1, device=self.device) < a:
#                 inner_mh += 1
#                 log_posterior_proposal_inner = self.posterior_distribution_inner(theta_proposal)

#                 # Save results of the values of the surrogates and solver and proposed thetas
#                 theta_inner[inner_mh-1,:] = theta_proposal.clone()
#                 likelihoods_val_nn[inner_mh-1,:] = self.outer_likelihood_value.T.clone()
#                 likelihoods_val_solver[inner_mh-1,:]  = self.inner_likelihood_value.T.clone()
#                 # Compute the acceptance ratio
#                 a = torch.clamp(torch.exp(log_posterior_proposal_inner - log_posterior_inner)*(a_rec/a), max=1.0)

#                 if verbose:
#                     pbar.update(1)

#                 if torch.rand(1, device=self.device) < a:
#                     theta = theta_proposal.clone()
#                     inner_accepted += 1
#                     acceptance_list[inner_mh-1] +=1

#                     log_posterior_outer = log_posterior_proposal_outer
#                     log_posterior_inner = log_posterior_proposal_inner

#             # if samples:
#             #     # Store the current sample and step size
#             #     samples_inner.append(theta)

#             # Update progress bar and print progress every 10% (or any interval)
#             if verbose and (inner_mh % (self.iter_da // 10) == 0) and (inner_mh != 0):
#                 acceptance_rate = inner_accepted / inner_mh
#                 pbar.set_postfix(acceptance_rate=f"{acceptance_rate:.4f}")

#         # time_inner = inner_timer.stop()
#         # End progress bar
#         if verbose:
#             pbar.close()
#             print(f"Times inner step {inner_mh:.4f}, Acceptance Rate: {inner_accepted / inner_mh:.4f}")

#         return samples_outer,acceptance_list.cpu(),theta_inner.cpu(),likelihoods_val_nn.cpu(),likelihoods_val_solver.cpu()
    

            

# class MoreauYosidaPrior(Distribution):
#     def __init__(self, lam, batch_shape=torch.Size([]), device='cpu'):
#         super().__init__()
#         self.device = device
#         self.batch_shape = torch.Size(batch_shape)  # Ensure batch_shape is a torch.Size object
#         self.lam = torch.tensor(lam, dtype=torch.float32, device=self.device)


#     @property
#     def event_shape(self):
#         """The event shape for this distribution is 1-dimensional."""
#         return torch.Size([1])

#     @property
#     def support(self):
#         """The support of the distribution is the whole real line."""
#         return dist.constraints.real

#     def log_prob(self, x):
#         """Calculate the log probability of x under the Moreau-Yosida prior."""
#         if not isinstance(x, torch.Tensor):
#             x = torch.tensor(x, dtype=torch.float32, device=self.device)

#         # Calculate the regularization term
#         regularization_term = -(torch.clamp(torch.abs(x) - 1, min=0) ** 2) / (2 * self.lam)
#         log_prob = regularization_term - torch.log(2 + torch.sqrt(2 * torch.pi * self.lam))

#         return log_prob

#     def sample(self, sample_shape=torch.Size()):
#         """Sample from the Moreau-Yosida prior using importance sampling."""
#         sample_shape = torch.Size(sample_shape)  # Ensure sample_shape is a torch.Size object
#         total_shape = self.batch_shape + sample_shape  # Concatenate batch_shape with sample_shape

#         # Proposal distribution with heavier tails (e.g., Normal distribution)
#         proposal_dist = dist.Normal(loc=0.0, scale=2.0)  # Wider scale for more tail coverage
#         proposals = proposal_dist.sample(total_shape).to(self.device)

#         # Calculate log probabilities for the proposals under the Moreau-Yosida prior
#         log_prob_samples = self.log_prob(proposals)

#         # Calculate log probability of the proposal distribution
#         log_prob_proposal = proposal_dist.log_prob(proposals)

#         # Calculate weights for importance sampling
#         weights = torch.exp(log_prob_samples - log_prob_proposal)

#         # Clamp weights to avoid negative entries
#         weights = torch.clamp(weights, min=0)

#         # Normalize weights
#         weights_sum = torch.sum(weights)
#         if weights_sum > 0:  # Prevent division by zero
#             weights /= weights_sum
#         else:
#             # If all weights are zero, return uniformly sampled values
#             return proposals

#         # Resample according to weights (multinomial resampling)
#         idx = torch.multinomial(weights, total_shape[0], replacement=True)
#         resampled_proposals = proposals[idx]

#         return resampled_proposals

#     def expand(self, batch_shape, _instance=None):
#         """Expand the distribution to a new batch shape."""
#         return MoreauYosidaPrior(self.lam, batch_shape, self.device)