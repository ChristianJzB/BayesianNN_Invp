...
def elliptic2d_neural_training(N=2500,hidden_layers=3,num_neurons=100,batch_size=250,kl=2,dgala=True,marginal=False):
    config = ConfigDict()
    config.model_type = "dgala"

    config.paths = ConfigDict()
    config.paths.base_dir = "./elliptic2D"
...
def elliptic2d_pigp_experiment(kernel_spatial ="MaternKernel52",kernel_parameter="SquaredExponential",N=10,spatial_points=6,kl=2,marginal=False):
    config = ConfigDict()
...
def elliptic2d_inverse_problem(model_type,verbose,dgala=True,mcmc=False,da=True,marginal_approximation=False,alpha_eval=True,kl=2,
                            proposal="random_walk",mcmc_samples = 1_000_000,da_eval = 50_000, repeat =3):
    config = ConfigDict()
...
def elliptic2d_gp_experiment(kernel = "MaternKernel52",N=10,spatial_points=6,kl=2,marginal=False):
    config = ConfigDict()
