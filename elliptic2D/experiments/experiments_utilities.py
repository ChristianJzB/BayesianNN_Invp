...
from elliptic2d_files.elliptic2d_pigp import EllipticPIGP
...
def build_paths(config):
    ...
    if model_type == "dgala":
        ...
        # NN paths
        paths = {
            "dgala_mean_model": os.path.join(config.paths.model_dir, f"dgala2d_mean{model_id}.pth"),
            "dgala_marginal_model": os.path.join(config.paths.model_dir, f"dgala2d_marginal{model_id}.pth"),
            "dgala_mcmc": os.path.join(config.paths.results_dir, f"dgala2d_mcmc_{aprox_type}{model_id}" + "_{i}.npy"),
            "dgala_da": os.path.join(config.paths.results_dir, f"dgala2d_da_{aprox_type}{model_id}" + "_{i}.npy"),
            "dgala_da_chain": os.path.join(config.paths.results_dir, f"dgala2d_da_chain_{aprox_type}{model_id}" + "_{i}.npy"),
            "dgala_mean_proposal": os.path.join(config.paths.results_dir, f"dgala2d_mean_proposal{model_id}.npy"),
            }
        ...
        paths ={"pigp_model": os.path.join(config.paths.model_dir,f"pigp2d{model_id}.pth"),
                "pigp_mcmc":os.path.join(config.paths.results_dir,f"pigp2d_mcmc_{aprox_type}{model_id}" + "_{i}.npy"),
                "pigp_da":os.path.join(config.paths.results_dir,f"pigp2d_da_{aprox_type}{model_id}" + "_{i}.npy"),
                "pigp_da_chain": os.path.join(config.paths.results_dir, f"pigp2d_da_chain_{aprox_type}{model_id}" + "_{i}.npy"),
                "pigp_mean_proposal":os.path.join(config.paths.results_dir, f"pigp2d_mean_proposal{model_id}.npy")}
        ...
        paths ={{"gp_model": os.path.join(config.paths.model_dir,f"gp2d{model_id}.pth"),
                 "gp_mcmc":os.path.join(config.paths.results_dir,f"gp2d_mcmc_{aprox_type}{model_id}" + "_{i}.npy"),
                 "gp_da_mcmc": os.path.join(config.paths.results_dir,f"gp2d_da_mcmc_{aprox_type}{model_id}" + "_{i}.npy"),
                 "gp_mean_proposal":os.path.join(config.paths.results_dir, f"gp2d_mean_proposal{model_id}.npy"),}}
    
    elif model_type == "fem":
        model_id = (f"_kl{config.KL_expansion}")
        
        paths = {"fem_mcmc": os.path.join(config.paths.results_dir,f"FEM2d_mcmc{model_id}" + "_{i}.npy"),
                 "fem_mcmc_eval":os.path.join(config.paths.results_dir,f"FEM2d_mcmc_eval{model_id}" + "_{i}.npy")}

    return paths
    
def train_neural_network(config_experiment,path,device):
    model_specific = f"2d_mean_hl{config_experiment.model.num_layers}_hd{config_experiment.model.hidden_dim}_s{config_experiment.nn_samples}_b{config_experiment.batch_size}_kl{config_experiment.KL_expansion}"
    ...
    pinn_ell = train_elliptic2d(config_experiment, device=device)
    ...
    if config_experiment.dgala:
        ...
        nn_model.eval()
        data_fit = deepgala_data_fit(config_experiment.nn_samples,config_experiment.KL_expansion,device, seed=config_experiment.seed)
        llp = dgala(nn_model)
        llp.fit(data_fit)
...
def train_pigp(config_experiment,path,device):    
    ...
    data_training,_,_ = pigp_training_data_generation(config_experiment.observed_solutions,config_experiment.observed_spatial_points,
                                                      config_experiment.KL_expansion,device,seed = config_experiment.seed)

    elliptic_gp = EllipticPIGP(data_training,kernel_spatial=config_experiment.kernel_spatial,
...
def get_models(config, config_model,paths, device):
    if config.model_type == "dgala":
        ...
        model_name = "fem"
        model = Elliptic2D_FEM(np.zeros(config.KL_expansion), ncells=config.FEM_h)
