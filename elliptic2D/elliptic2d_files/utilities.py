...
from .elliptic2d_mcmc import  EllipticMCMCDA
...
def generate_noisy_obs(obs, nparam=2,ncells=50, mean=0, std=np.sqrt(1e-4), seed = 42):
    ...
    noise_sol_points = add_noise(sol_points, mean, std, seed=seed)
...
def pigp_training_data_generation(theta_obs,spatial_points_obs,kl_expansion,device,noise_level=1e-4,ncells=50,seed = 65647437836358831880808032086803839626):
    rng = np.random.default_rng(seed)

    obs_points, sol_test,_ = generate_noisy_obs(obs=spatial_points_obs,std=np.sqrt(noise_level),nparam=kl_expansion,ncells=ncells, seed=seed)

    thetas = rng.uniform(low = -3,high = 3,size=(theta_obs,kl_expansion))
