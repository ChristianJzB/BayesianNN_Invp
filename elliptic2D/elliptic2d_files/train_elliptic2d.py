...
from .elliptic2d_neural import Elliptic2D

def samples_param(size, nparam, min_val=-4, max_val=4,seed = 65647437836358831880808032086803839626):
    """Sample parameters uniformly from a specified range."""
    rng = np.random.default_rng(seed)
    return rng.uniform(min_val, max_val, size=(size, nparam))

class dGDataset(Dataset):
    def __init__(self, size,param = None,nparam = 2,seed=65647437836358831880808032086803839626):
        self.nparam = nparam
        self.size = size

        self.data_int, self.left_bc, self.right_bc,self.down_bc,self.up_bc = self.generate_data(size= size, param = param, seed=seed)

    def generate_data(self, size, seed, param = None):
        #x = lhs(1, size).reshape(-1, 1)  # Latin Hypercube Sampling for x
        sampler = qmc.LatinHypercube(d=2, seed=seed)
        x = sampler.random(n=size)

        if param is None:
            param = samples_param(size=size, nparam=self.nparam, seed=seed)
        else:
            param = param[:size,:]
...
def generate_test_data(size,param = None,ncells=50, nparam=2, seed = 42):
    """
    ...
    # Sample parameters
    if param is None:
        test_samples_param = samples_param(size, nparam=nparam, seed = seed)
    else:
...
def train_elliptic2d(config, device):
    ...
    data_parameters = samples_param(config.nn_samples*2, nparam=config.KL_expansion, seed=config.seed)
    
    param_train, param_test = data_parameters[:config.nn_samples,:],  data_parameters[config.nn_samples:,:]

    dataset = dGDataset(size = config.nn_samples, param=param_train,seed=config.seed)
