...
class EllipticMCMCDA(MCMCDA):
    ...
    def __init__(self, surrogate, finer_surrogate,observation_locations, observations_values, nparameters=2, 
                 marginal=False, observation_noise=0.5, iter_mcmc=1000000, iter_da = 20000, burnin=None, 
                 proposal_type="random_walk",prior_type = "uniform",uniform_limit=1,my_reg = 1e-3, 
                 step_size=0.1,device="cpu"):
        
        super(EllipticMCMCDA, self).__init__(observation_locations, observations_values, nparameters, marginal,
                 observation_noise, iter_mcmc,iter_da, burnin, proposal_type,prior_type, uniform_limit,
                 my_reg,step_size,device)

        self.surrogate_model = surrogate
        self.fine_model = finer_surrogate
    
        if marginal:
            methods = {Elliptic1DPIGP: self.gp_marginal_model,
                        dgala: self.dgala_marginal_model}
        else:
            methods = {FEMSolver: self.fem_model,
                        Elliptic1DPIGP: self.gp_mean_model,
                        Elliptic: self.dgala_mean_model}

        surrogate_type = type(surrogate)
        if surrogate_type in methods:
            self.outer_model = methods[surrogate_type]
        else:
            raise ValueError(f"Surrogate of type {surrogate_type.__name__} is not supported.")
        self.finner_model = self.fem_model

    def fem_model(self,theta):
        """Evaluates the log-likelihood given a FEM."""
        self.fine_model.theta = theta.cpu().numpy()  # Convert to numpy for FEM solver
        self.fine_model.solve()
        surg = self.fine_model.evaluate_at_points(self.observation_locations.cpu().numpy()).reshape(-1, 1)
        surg = torch.tensor(surg, device=self.device)
        return surg
    
    def gp_mean_model(self,theta):
        mean_surg = self.surrogate_model.prediction(theta.reshape(1,-1),var=False)
        return mean_surg
    
    def gp_marginal_model(self,theta):
        mean_surg, var_surg = self.surrogate_model.prediction(theta.reshape(1,-1),var=True)
        return mean_surg.reshape(-1, 1), var_surg
    
    def dgala_mean_model(self,theta):
        theta_expanded = theta.unsqueeze(0).expand(self.nobservations, -1)
        data = torch.cat([self.observation_locations, theta_expanded], dim=1)
        #data = torch.cat([self.observation_locations, theta.repeat(self.observation_locations.size(0), 1)], dim=1)
        mean = self.surrogate_model.u(data.float()).detach()
        return mean.reshape(-1, 1).double()

    def dgala_marginal_model(self,theta):
        theta_expanded = theta.unsqueeze(0).expand(self.nobservations, -1)
        data = torch.cat([self.observation_locations, theta_expanded], dim=1)
        #data = torch.cat([self.observation_locations, theta.repeat(self.observation_locations.size(0), 1)], dim=1)
        mean, cov = self.surrogate_model(data.float())
        return mean.reshape(-1, 1).double(), cov[:, :, 0].double()
...
#             MultiOutputRegressor: self.gp_scikit_log_likelihood,
#             MultioutputGP:self.gp_log_likelihood}
...
#         sigma = var_surg + torch.eye(dy, device=self.device) * self.observation_noise ** 2
...
#     def gp_scikit_log_likelihood(self,theta):
#         """
#         Evaluates the log-likelihood given a GP.
#         """
#         mean_surg = self.surrogate.predict(theta.numpy().reshape(1,-1))
#         surg = torch.tensor(mean_surg, device=self.device).reshape(-1, 1)
#         return -0.5 * torch.sum(((self.observations_values - surg) ** 2) / (self.observation_noise ** 2)),surg
...
#         surg_sigma = surg_sigma[:, :, 0]

#         dy = surg_mu.shape[0]

#         diff = (self.observations_values - surg_mu)

#         sigma = surg_sigma +  torch.eye(dy, device=self.device) *  self.observation_noise ** 2
...
#     # def gp_scikit_log_likelihood(self,surrogate,theta):
#     #     """
#     #     Evaluates the log-likelihood given a GP.
#     #     """
#     #     mean_surg = surrogate.predict(theta.numpy().reshape(1,-1))
#     #     surg = torch.tensor(mean_surg, device=self.device).reshape(-1, 1)
...
#         self.outer_likelihood_value = mean_surg.reshape(-1, 1)
...
#         surg_mu = surg_mu.view(-1, 1)
#         surg_sigma = surg_sigma[:, :, 0].detach()
