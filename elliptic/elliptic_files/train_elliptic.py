...
from .elliptic_neural import Elliptic
from Base.utilities import UniformSampler

def samples_param(size, nparam, min_val=-2, max_val=2,seed = 65647437836358831880808032086803839626):
...
class dGDataset(Dataset):
    ...
    def generate_data(self, size, param = None, seed = 65647437836358831880808032086803839626):
        ...
        data_int = torch.cat([x_tensor, param_tensor], axis=1)
        left_bc = torch.cat([torch.zeros_like(x_tensor), param_tensor], axis=1)
        right_bc = torch.cat([torch.ones_like(x_tensor), param_tensor], axis=1)
