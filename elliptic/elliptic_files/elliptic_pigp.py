...
from Base.gp_base import PIGPBase

class Elliptic1DPIGP(PIGPBase):
    def __init__(self, data_training, reg_matrix=1e-6, kernel_spatial = "Matern52",kernel_parameter = "SquaredExponential",
        ...
        super().__init__(data_training = data_training , reg_matrix=reg_matrix,kernel_spatial=kernel_spatial,
                         kernel_parameter=kernel_parameter,sigma_l_parameters=sigma_l_parameters,
                           sigma_l_spatial=sigma_l_spatial,device=device)
        # Roots for KL
        self.finder = RootFinder(1/4, self.parameter_dim)
    ...
    def g_training(self):
        ...
        return torch.cat([self.solutions_data.view(-1, 1), y_bc_n, f_source_n]).detach()
    ...
    def kernel_uf(self, theta, x):
        ...
        dr_dx = self.kernel_spatial.d_euclidean_distance(x, self.source_func_x)
    ...
    def kernel_ff(self, theta1, theta2):
        ...
        dr_dx1 = self.kernel_spatial.d_euclidean_distance(self.source_func_x, self.source_func_x)
...
# class MaternKernel_32(KernelFunction):
#     def __init__(self, sigma=1.0, l=1.0, device='cpu'):
#         super().__init__(device)
#         self._sigma = torch.tensor(sigma, dtype=torch.float64, device=device)
#         self._l = torch.tensor(l, dtype=torch.float64, device=device)
...
# class VanillaGP:
#     def __init__(self, data_training, reg_matrix=1e-6,sigma_l_parameters=(1, 1), device = "cpu"):
#         self.device = device
#         # Training data
...
#         cov += self.reg_matrix * torch.eye(cov.shape[0], dtype=torch.float64, device=self.device)
#         L = torch.linalg.cholesky(cov)
#         return cov,L
    
#     def g_training(self):
#         return self.solutions_data.view(-1, 1)

#     def kernel_inverse(self, L, Y):
#          return  torch.cholesky_solve(Y, L)

#     def marginal_likelihood(self, sigma_param, l_param):
...
#         params = [log_sigma_param, log_l_param]
...
#             # reg_matrix = log_reg_matrix.exp()

#             nll = -self.marginal_likelihood(sigma_param, l_param)
#             nll.backward()
#             return nll
...
#     def optimize_gps(self):
#         [gp_model.optimize_mll() for gp_model in self.GPs]
...
#         self.kernel_parameter = MaternKernel(*sigma_l_parameters,device=self.device)
#         self.kernel_spatial = torch.eye(self.solutions_data.shape[-1], dtype=torch.float64,device=self.device)
...
#         cov = torch.kron(kernel_param,self.kernel_spatial) 
#         cov += self.reg_matrix * torch.eye(cov.shape[0], dtype=torch.float64, device=self.device)
#         L = torch.linalg.cholesky(cov)
#         return cov,L
...
#     def marginal_val(self, theta_test):
#         kernel_param = self.kernel_parameter.covariance(theta_test, self.parameters_data)
#         return torch.kron(kernel_param,self.kernel_spatial )
...
#             kinv_y = self.kernel_inverse(self.L, matrix_test.T)
#             cov = torch.kron(kernel_param, self.kernel_spatial) - matrix_test @ kinv_y
#             return marginal_mean, cov
#         return marginal_mean
...
#         self.kernel_parameter = MaternKernel_32(*sigma_l_parameters,device=self.device)
...
#         self.reg_matrix = torch.tensor(reg_matrix, dtype=torch.float64, device=device)
...
#     def marginal_likelihood(self, sigma_spatial, l_spatial,l_param):
#         # Update kernel parameters
#         self.kernel_spatial.sigma, self.kernel_spatial.l = sigma_spatial, l_spatial
#         self.kernel_parameter.l = l_param
#         # self.reg_matrix = reg_matrix
...
#         log_l_spatial = self.kernel_spatial.l.detach().log().clone().requires_grad_()
#         log_l_param = self.kernel_parameter.l.detach().log().clone().requires_grad_()
#         #log_reg_matrix =  self.reg_matrix.detach().log().clone().requires_grad_()

#         params = [log_sigma_spatial, log_l_spatial,log_l_param]
#         optimizer = torch.optim.LBFGS(params, lr=lr, max_iter=100, line_search_fn='strong_wolfe')
...
#             nll = -self.marginal_likelihood(sigma_spatial, l_spatial,l_param)
#             nll.backward()
#             return nll
